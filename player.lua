...
function player:update(dt)
  ...
  self.circle_x = self.x + 60
  self.circle_y = self.y
  if self.has_swiping then
    self.actual_angle = self.actual_angle + DIFF_ANGLE
    self.circle_x = self.circle_x + (60 * self.step)
    self.circle_y = self.circle_y + (-10 * self.step)
    self.step = STEP + self.step
    if self.step > 1.0 then self.step = 0.0 end
...
function player:draw()
  ...
  if self.has_racquet then
    ...
    self.racquet_body:setPosition(self.circle_x, self.circle_y)
